...
int CStringBuilder::Replace (const char* psz, const char *pOld, const char *pNew)
{
	int nResult = 0;
...
void CStringBuilder::Format (const char *pFormat, ...)
{
	va_list var;
...
void CStringBuilder::FormatV (const char *pFormat, va_list Args)
{
	free(m_pBuffer);
...
void CStringBuilder::PutChar (char chChar, size_t nCount)
{
	ReserveSpace (nCount);
...
void CStringBuilder::PutString (const char *pString)
{
...
void CStringBuilder::ReserveSpace (size_t nSpace)
{
	if (nSpace == 0)
	{
		return;
...
char *CStringBuilder::ntoa (char *pDest, unsigned long ulNumber, unsigned nBase, boolean bUpcase)
{
...
#if STDLIB_SUPPORT >= 1
char *CStringBuilder::lltoa (char *pDest, unsigned long long ullNumber, unsigned nBase, boolean bUpcase)
{
...
char *CStringBuilder::ftoa (char *pDest, double fNumber, unsigned nPrecision)
{
	char *p = pDest;
...
void CStringBuilder::Attach(char* pString)
{
	free(m_pBuffer);
...
int string_replace(char** pVal, const char* psz, const char* pOld, const char* pNew)
{
	...
	int retv = builder.Replace(psz, pOld, pNew);
	*pVal = builder.Detach();
	return retv;
}
